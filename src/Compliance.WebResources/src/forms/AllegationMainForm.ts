...
export namespace Allegation.Forms {
    ...
    export class MainForm implements IPowerForm<Form.opc_allegation.Main.Information> {
        ...
        public initializeComponents(initializationContext: Xrm.ExecutionContext<Form.opc_allegation.Main.Information, any>): void {
            ...
            formContext.getAttribute("opc_dispositionreason").addOnChange(x => this.dispositionreason_OnChange(x));
            formContext.getAttribute("opc_dispositionreason").fireOnChange();

            formContext.getAttribute("opc_dispositionreason").controls.forEach(x => x.addPreSearch(() => this.dispositionreason_PreSearch(formContext)));
            formContext.getAttribute("opc_dispositionaction").controls.forEach(x => x.addPreSearch(() => this.dispositionaction_PreSearch(formContext)));
        ...
        private dispositionreason_OnChange(context: Xrm.ExecutionContext<Xrm.LookupAttribute<"opc_dispositionreason">, undefined>): any {
            ...
            let value = formContext.getAttribute("opc_dispositionreason").getValue();
            ...
            if (value) {
                ...
                let isActionAvailable = [/* not a privacy complaint */ "{11DF9980-A76E-EA11-A811-000D3AF45A96}"] 
                    .includes(value[0].id);
                formContext.getAttribute("opc_dispositionaction").controls.forEach(c => XrmHelper.turn(c, isActionAvailable));

            } else {
                formContext.getAttribute("opc_dispositionaction").controls.forEach(c => XrmHelper.turnOff(c));
            }
        ...
        private disposition_OnChange(context: Xrm.ExecutionContext<Xrm.OptionSetAttribute<opc_allegationdisposition>, undefined>): any {
            ...
            formContext.getAttribute("opc_dispositionreason").controls.forEach(c => XrmHelper.turn(c, isReasonAvailable, !this._initializing));
        ...
        private dispositionaction_PreSearch(formContext: Form.opc_allegation.Main.Information) {
            formContext.getAttribute('opc_dispositionaction').controls.forEach(
                x => x.addCustomFilter(this._allegationService.getAllegationDispositionActionFilter(formContext.getAttribute("opc_dispositionreason").getValue()[0].id), 'opc_dispositionaction')
            );
        }
