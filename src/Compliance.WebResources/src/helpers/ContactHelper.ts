...
export class ContactHelper {
    ...
    static getMostLikelyDuplicate(contact: Contact_Result, potentialDuplicates: IBaseContact[]): IPotentialDuplicate {
        let sortedPotentialDuplicates = potentialDuplicates
    ...
    static getNumberOfFieldMatches(contact: Contact_Result, potentialDuplicate: IBaseContact): IPotentialDuplicate {
        ...
        if (contact.telephone1 && (
            potentialDuplicate.telephone1 == contact.telephone1
            || potentialDuplicate.telephone1 == contact.telephone2
        ))
            numberOfFieldMatches++;
        if (contact.telephone2 && (
            potentialDuplicate.telephone2 == contact.telephone1
            || potentialDuplicate.telephone2 == contact.telephone2
        ))
            numberOfFieldMatches++;
        if (contact.address1_postalcode && potentialDuplicate.address1_postalcode == contact.address1_postalcode)
            numberOfFieldMatches++;

        let potentialDuplicateWithMatches = <IPotentialDuplicate>potentialDuplicate;
