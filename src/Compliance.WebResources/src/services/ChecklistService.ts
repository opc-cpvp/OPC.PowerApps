...
export class ChecklistService implements IChecklistService {
    ...
    getChecklist(id: string): Promise<({ opc_questiontemplateid: opc_QuestionTemplate_Result; } & opc_ChecklistResponse_Result)[]> {
        ...
        return XrmQuery.retrieveMultiple(x => x.opc_checklistresponses)
            ...
            .expand(x => x.opc_questiontemplateid, x => [x.opc_questiontemplateid, x.opc_name, x.opc_conditionalvisibility, x.opc_sequence, x.opc_questiontypeid_guid, x.opc_parentquestiontemplateid_guid, x.opc_nameenglish, x.opc_namefrench, x.opc_additionalparameters, x.opc_managedinternally])
