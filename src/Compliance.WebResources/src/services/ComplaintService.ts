...
export class ComplaintService implements IComplaintService {
    ...
    getComplaintWithRelationships(id: string): Promise<ComplaintWithRelationships> {
        const contactAttributes = (x: Contact_Select): WebAttribute<Contact_Select, any, any>[] => [x.firstname, x.lastname, x.fullname, x.jobtitle, x.emailaddress1, x.telephone1, x.telephone2, x.fax, x.address1_line1, x.address1_line2, x.address1_line3, x.address1_city, x.address1_stateorprovince, x.address1_postalcode, x.address1_country];
        const accountAttributes = (x: Account_Select): WebAttribute<Account_Select, any, any>[] => [x.name, x.telephone1, x.fax, x.websiteurl, x.address1_line1, x.address1_line2, x.address1_line3, x.address1_city, x.address1_stateorprovince, x.address1_postalcode, x.address1_country];
        const userAttributes = (x: SystemUser_Select): WebAttribute<SystemUser_Select, any, any>[] => [x.firstname, x.lastname, x.fullname, x.title, x.domainname, x.internalemailaddress, x.address1_telephone1, x.mobilephone];
