...
export class SharePointService implements ISharePointService {
    async getTemplates(sharePointSiteUrl: string, templatesFolderPath: string, accessToken: string): Promise<any[]> {
        ...
        const request = new Request(
            `${sharePointSiteUrl}_api/web/GetFolderByServerRelativeUrl('${templatesFolderPath}')/Files`
            , {
                method: 'GET',
                headers: new Headers({
    ...
    async generateDocumentFromTemplate(accessToken: string, caseFolderPath: string, templatePath: string, xmlData: string, documentName: string, sharePointSiteUrl: string): Promise<Xrm.WebApiResponse> {
        const request = {
